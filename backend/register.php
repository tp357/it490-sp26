...
if (is_array($response) && ($response['status'] ?? '') === 'success') {
    $loginResponse = $client->send_request(array(
        'type' => 'login',
        'username' => $input['username'],
        'password' => $input['password']
    ));
    $ok = is_array($loginResponse) && ($loginResponse['status'] ?? '') === 'success';
    http_response_code($ok ? 200 : 401);
    echo json_encode(is_array($loginResponse) ? $loginResponse : array('status' => 'error', 'message' => 'registration succeeded but auto-login failed'));
} else {
